...
namespace ShibaHelper
{
    public class ShibaHelpers
    {
        ...
        public static bool searchLayerNames(ChildAnimatorState[] checkStates)
        {
            for (int i = 0; i < checkStates.Length; i++)
            {
                if (checkStates[i].state.name == "Default DT State")
                    return true;
            }
            return false;
        }
